...
gluPerspective(75.0, asp, 0.25, 256.0)
### P R O J E C T I O N ###\
...
class WorldModelSet(object):
    ...
    def build_polygon_topography(self, method, amt, amto):
        ...
        if method is 'nodal_random':
            ...
            radius = (model_world_radius * pi * 2) / (model_world_density*amt)
            print('radius',radius)
...
class AggroEntityHandler(X29Element):
    ...
    def __init__(self, name):
        ...
        super(AggroEntityHandler, self).__init__(name, er)
        self.mass = er * 100.0
        self.startmass = er * 100.0
        self.type = 'aggro'
        self.radius = 1.0
...
def generate_aggro_entities(howmany):
    """the class of red blobby npcs"""
    for l in range(0, howmany):
...
tsec = 0.0
# ||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||
instruct = {1:instructions,2:instructions_2,3:instructions_3}
...
def x29_runtime(timedelta, has_input):
    ...
    if run_x29.info:
        ...
        if run_x29.info == len(instruct): run_x29.info = 0
