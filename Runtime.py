...
globals()['instructions'] = "(level 1 of instructions)\n(key): action\n"
for tupl in group:
    #"%s:\t%s\n"
    kstr = ("( %s ): %s\n" % (tupl[0], tupl[1].replace('_',' ').replace('-',' '))).upper()
    instructions += kstr
instructions += "( ESC )( CMD-Q )( CTRL-Q ): QUIT\n( RETURN ): AUTO-DESTRUCT\nPress ? again for more"
globals()['instructions_2'] = "(level 2 of instructions)\nYour mission is to collect flags.\nYou should try and kill everything.\nEverything is trying to kill you.\nPress ? again for more"
globals()['instructions_3'] = "(level 3 of instructions)(last)\n\"I don\'t want these logs looking\nlike a Boy Scout was turned loose \non them with a dull hatchet\"-D.Proenneke\nPress ? again to close instructions"
globals()['level_messages'] = ["PREPARE YOUR SPACE",
                               "PREPARE YOUR ANUS",
                               "HAVE A COOKIE",
                               "HELMETS ON",
                               ...
                               "FIND AN EXIT ROW",
                               "UPGRADE TO ECONOMY",
                               "GROUND CONTROL CANNOT BE REACHED",
                               "YOUR BASE",
                               "SYSTEMWIDE BORK",
                               "EGG SALAD"]


def acquire_json_runtime_table(filenname):
    with open(filenname) as r: return json.load(r)
globals()['runtime_table'] = acquire_json_runtime_table( os.path.join(BASEDIR, 'Unknown.json') )
...
class BetterTypeHandler(object):
    ...
    def __init__(self,BD):
        self.BASEDIR = BD
        font_set = os.path.join(self.BASEDIR, 'pf_tempesta_seven.ttf')
        self.makefont(font_set, 16)
    ...
    def __call__(self, asnd, *args):
        ...
        if len(args) > 0:
            ...
            if args[0].__class__ == Vector3:
                ...
                if channel: channel.set_volume(psnd['basevolume']*LR[1], psnd['basevolume']*LR[0])
                return
            elif args[0] == 'stop':
                psnd['sound'].stop()
                return
...
snd.add('audio/small-fire-space-cake_1.aif', 'aggro-fire', 0.4)
snd.add('audio/aggro-speak-0.aif', 'aggro-speak-0', 0.3)
snd.add('audio/aggro-speak-1.aif', 'aggro-speak-1', 0.3)
snd.add('audio/aggro-speak-2.aif', 'aggro-speak-2', 0.3)
snd.add('audio/aggro-speak-3.aif', 'aggro-speak-3', 0.3)
snd.add('audio/aggro-drone.aif', 'aggro-drone', 0.3)

snd.add('audio/02 Nasty Spell_3_bip.wav', 'welcome', 0.6)
snd.add('audio/explode-3.wav', 'explode', 0.85)
snd.add('audio/explode-mini.aif', 'explode-mini', 0.3)
...
if fullscreen:
    srf = pygame.display.set_mode(viewport, FULLSCREEN | OPENGL | DOUBLEBUF | HWSURFACE | NOFRAME, 0)
else:
...
glLightfv(GL_LIGHT1, GL_DIFFUSE, (0.75, 1.0, 0.75, 1.0))
glLightfv(GL_LIGHT1, GL_SPECULAR, (0.75, 1.0, 0.75, 1.0))
glLightfv(GL_LIGHT1, GL_CONSTANT_ATTENUATION, (1.0))
glLightfv(GL_LIGHT1, GL_LINEAR_ATTENUATION, (0.01))
glLightfv(GL_LIGHT1, GL_QUADRATIC_ATTENUATION, (0.00125))
...
gluPerspective(75.0, asp, 0.25, 128.0)
### P R O J E C T I O N ###\
...
#// COUNT SHIELDs SHIELD_AMT DMG CTK SHOOTS TARGETS

#level_behavior_table = {[]}
...
class WorldModelSet(object):
    ...
    def build_stars(self,howmany):
        ...
        for n in range(0, howmany):
            ...
            cc = model_world_radius * 2 * starpos.unit()
    ...
    def build_polygon_topography(self, method, amt, amto):
        ...
        if method is 'nodal_random':
            self.SELECTED_ACTT_FACES = []
            heightmax = 1.0+float(current_level)
            wall_height_max = 3.0 #3 possible steps

            radius = (model_world_radius * pi * 2) / (2.0*amt) #(model_world_density*amt)
            print('radius',radius)

            for ev in range(amt): self.SELECTED_VERTS.append(random.randrange(0, len(self.obj.vertices)))
            for ev in range(amt): self.SELECTED_APEX_VERTS.append(random.randrange(0, len(self.obj.vertices)))
            for ef in range(amt*3): self.SELECTED_ACTT_FACES.append(random.randrange(0, len(self.obj.faces)))

            vertices, normals, texture_coords, material = self.obj.faces[0]
            a = Vector3(self.obj.vertices[vertices[0]-1])
            b = Vector3(self.obj.vertices[vertices[1]-1])
            avgd = ((a.unit()-b.unit()).length)*(0.5 * model_world_radius)*model_world_density

            """pass below builds mesas"""
            for vertexid in self.SELECTED_APEX_VERTS:
                ...
                for vertex in self.obj.vertices:
                    v0 = Vector3(vertex)
                    d = ((vS.unit() - v0.unit()).length)*(0.5*model_world_radius)*model_world_density
                    #batch.append(d)
                    if d < radius:
                        if d <= avgd*1.5:
                            ed = (radius / (1.5*avgd))
                            nv = ed * heightmax
                            pvertex = v0.unit() * (vS.length + nv)
                        else:
                            if d > 0:
                                ed = (radius / d) - (d/(model_world_radius/heightmax))
                                nv = ed * heightmax
                            pvertex = v0.unit() * (v0.length + nv)

                        self.obj.vertices[index] = (pvertex.x, pvertex.y, pvertex.z)

                    index += 1
                #print('max distance:',max(batch))

            """pass to build walls"""
            #3index = 0
            for face in self.SELECTED_ACTT_FACES:
                vertices, normals, texture_coords, material = self.obj.faces[face]
                htmod = (random.random()*wall_height_max)
                mean = min([Vector3(self.obj.vertices[bv-1]).length for bv in vertices])
                for v in vertices:
                    vc = Vector3(self.obj.vertices[v-1])
                    pvertex = (mean + htmod)*vc.unit()
                    self.obj.vertices[v-1] = (pvertex.x, pvertex.y, pvertex.z)
        ...
        for face in self.obj.faces:
            ...
            self.POLY_CENTERS_ARRAY[index]['center'] = vXX
            self.POLY_CENTERS_ARRAY[index]['normal'] = getNormal(v0, v1, v2)
    ...
    def build_polygon_centers(self):
        ...
        for face in self.obj.faces:
            ...
            if first_run: self.POLY_CENTERS_ARRAY[index]['local_faces'] = []
        ...
        if first_run:
            ...
            for ii in self.POLY_CENTERS_ARRAY:
                ...
                dc = self.POLY_CENTERS_ARRAY[ii]['center']
                self.POLY_CENTERS_ARRAY[ii]['nearest'] = list(set([mm for dd in vst for mm in self.SUB_SUPER_MEGA[str(dd)].values()]))

                for jj in range(0, user_poly_radius):
                    vst = list(set(vst+[int(ce) for bv in vst for ce,i in self.SUB_SUPER_MEGA[str(bv)].items()]))

                self.POLY_CENTERS_ARRAY[ii]['local_faces'] = list(set([mm for dd in vst for mm in self.SUB_SUPER_MEGA[str(dd)].values()]))
                #self.POLY_CENTERS_ARRAY[ii]['local_faces'].sort(key=lambda fe: (self.POLY_CENTERS_ARRAY[fe]['center']-dc).length)



                # self.POLY_CENTERS_ARRAY[ii]['local_faces'] = sorted(list(set([mm for dd in vst for mm in self.SUB_SUPER_MEGA[str(dd)].values()])),
                #                                                     key=lambda fe: (self.POLY_CENTERS_ARRAY[fe]['center']-dc).length)
            ...
            snd('affirm-beep',0.5)
            #print(self.POLY_CENTERS_ARRAY[ii]['local_faces'])
        fl = len(self.POLY_CENTERS_ARRAY)
        StackTrace("first_run:"+str(first_run)+" POLY_CENTERS_ARRAY", fl, self.POLY_CENTERS_ARRAY[fl - 1])

        la = len(self.POLY_CENTERS_ARRAY) - 1

        #self.POLY_CENTERS_ARRAY[la]['normal'] *= -1.0 #Vector3(0.0, 0.0, 0.0)
        #self.POLY_CENTERS_ARRAY[0]['normal'] *= -1.0 # = Vector3(0.0, 0.0, 0.0)
    ...
    def init_world_model_bases(self):
        ...
        StackTrace('world contains %i Faces | %i Normals | %i Vertices' % (len(self.obj.faces),len(self.obj.normals),len(self.obj.vertices)))

        self.build_polygon_centers()
        StackTrace("finished addresses")

        self.build_stars(stars_count)
        StackTrace("finished building stars")


    def load_world(self, apex_nodes, level):
        self.SELECTED_APEX_VERTS = []
        Upgrade.Upgrades_All = []
        StackTrace("building topography")

        self.build_polygon_topography('nodal_random', apex_nodes, 0.02)
        StackTrace("finished topography")

        self.obj.refresh()
        StackTrace("finished rebuilding world")
...
class X29Element(object):
    ...
    def setup(self, variance):
        self.vary = variance
        vary_mod = random.random()*variance

        crit = ('init_shield','htk','dmg')
        for cr in crit:
            ee = getattr(self,cr)
            setattr(self,cr,ee+ee*vary_mod)

        #COLOR
        rgbv = npc_data[self.type]['rgb']
        self.rgb = (rgbv[0],rgbv[1],rgbv[2])
        #MASS
        m = npc_data[self.type]['mass']
        self.mass = m+(m*vary_mod)
        self.init_mass = self.mass
        #RADIUS
        r = npc_data[self.type]['radius']
        self.radius = r+(r*vary_mod)
        ...
        #print [(e,getattr(ENTT,e)) for e in crit]
    ...
    def fire(self):
        ...
        elif 'blob' in self.name:
            snd('enemy-fire',self._pos)
            ftype = None
            fspeed = basic_flame_speed
        elif 'aggro' in self.name:
            snd('aggro-fire', self._pos)
            ftype = None
            fspeed = basic_flame_speed #*2.0
        else:
            ftype = None
            fspeed = basic_flame_speed
    ...
    def flamed(self, flame_or_entt, *alt_hit):
        ...
        if alt_hit:
            ...
            if isinstance(flame_or_entt,(int,long)):# or flame_or_entt.name != 'mine':
                hit_type = 'flame'
                ...
                f_rel_vector = (self._pos - flame_or_entt._pos)
                if f_rel_vector.length > 0: self.PHYS.Velo += f_rel_vector*10.0
                flame = None
                dmg_amt = mine_damage_amount
        ...
        if self.shield > 0.0:
            #if self.ctk > self.htk: self.ctk = self.htk
            #self.shield_radius = self.radius+(self.shield/self.init_shield)
            if hit_type is not 'collision': SMOKES.gimme_a_fucking_smoke(flame._pos, int(fires/2.0), 3.0, 'fire')
        else:
            self.shield = 0.0
            self.ctk += dmg_amt
            if self.ctk > self.htk: self.ctk = self.htk
            ...
            self.mass = self.init_mass*(1.0 - self.ctk / self.htk)



        if self.name == 'user':
            #print(self.shield,hit_type)
            ...
            if upgrades['insta_kill']['on'] or (self.ctk / self.htk > 0.9):
                if self.killed == 0:
                    # self.PHYS.set(self._pos*1.5)
                    # self.update(self.timeframe)
                    ...
                    if len(ttl) == len(self.ownerclass.PWEs)-1 and len(ttl) > 1:
                        ...
                        self.ownerclass.Waves_Killed += 1

                    for ele in self.ownerclass.PWEs:
                        if ele.name != self.name:
                            f_rel_vector = (ele._pos-self._pos)
                            ele.PHYS.Velo += (f_rel_vector.get_normalised()*10.0) / (abs(ele.mass) + 0.00001)
    ...
    def hit(self, sta, *by_obj):
        ...
        if self.name == 'user': self.flamed(by_obj[0], 1)
        #print(self.name,sta)
        if not sta: return
        if not self.hitting:
    ...
    def die(self):
        ...
        self.killed = 1
        self.active = 0
        self.PHYS.set_position(self._pos * 1.5)
        #self.move_entity(timepassed)
        self.ownerclass.Units_Killed += 1
        self.spawn_count += 1
        self.respawn_timer = ceil(random.random()*2000)
    ...
    def respawn(self):
        ...
        # self.mass = er * 10.0
        # self.init_mass = er * 10.0
        # self.dmg = er * current_level
        self.setup(self.vary)
        self.ctk = 0.0
    ...
    def update(self, timepassed):
        self.timeframe = timepassed
        self.position = self.PHYS.update(timepassed)
        self.ADDR._pos = self._pos
        self.ADDR._nextpos = self._nextpos
        self.ADDR.idle()

        if self.init_shield: self.shield_radius = self.radius + (self.shield/self.init_shield)

        attitude_deg = 0.0
        a_height = self.ADDR.alt
        a_normal = self.ADDR.nor.copy()
        """at_normal used by define_from_normal()"""
        self.at_normal = a_normal*-1.0
        vel = self.PHYS.Velo.length

        self.surface_position = (self._pos.length - a_height) * self._pos.get_normalised()
        self.altitude = a_height

        if (self.altitude > 3.0):self.at_normal = self._pos.get_normalised()
        ...
        if (self.altitude < 0.075) and not self.PHYS.Jumpy:
            ...
            if (attitude_deg > 55.0) and (vel > 3.0):
                ...
                if self.name == "user": snd('thud', 0.75)
...
class PolyAddressHandle(object):
    ...
    def update(self):
        fl = len(WORLD.POLY_CENTERS_ARRAY) - 1
        self.lastpoly = fl
        self.special = {0: self.get_warp, fl: self.get_warp}
        self.special_alt = {0:fl,fl:0}

    def get_warp(self, frompoly):
        #self.poly_a._nor = Vector3(0.0,0.0,0.0)
        topoly = self.special_alt[frompoly]
        GEN_ERR("warp to",topoly)
        self.poly_a = self.localize_poly(self.poly_a, topoly)

        pass

    def continue_pass(self,topoly):
        pass
    ...
    def check(self, poly, pos):
        ...
        return intersect_test(world_center, pos, poly._set[0], poly._set[1], poly._set[2])

        # if dzx: self.alt = dzx[0]
    ...
    def idle(self):
        ...
        if not p_current:
            ...
            if pid:
                self.message = str(self.last_poly) + '>' + str(pid)
                self.poly_a = poly
        ...
        if p_current:
            ...
            #self.real_alt = p_current[0]
            self.alt = self._pos.length - (p_current[0] * self._pos).length
            self.nor = self.poly_a._nor # p_current[1].get_normalised() #self.poly_a._nor*-1.0

        #self.special.get(self.at_poly, self.continue_pass)(self.at_poly)
        # if self.at_poly in self.special:
        #     #self.alt = self._pos.length
        #     #self.nor = self.poly_a._nor
        #     return self.special[self.at_poly](self.at_poly)
...
class Flame(object):
    ...
    class Bullet(object):
        ...
        def __init__(self, poly, has_target):
            ...
            self.interrupted = 0
            self.interruption_reflect = 0
            self.bounced_and_hit = 0
        ...
        def propagate(self):
            if self.target is not None:
                ...
                self.direction += (d.get_normalised()-self.direction)*0.1
                moveamt = self.direction*0.2
            ...
            # if 'Aggro' in self.shooter.name and self.frame == self.frames-1:
            #     print(self.ADDR.alt)
            ...
            if (self.ADDR.alt < 0.05) and self.target is not None:
                self.direction = self.reflect(self.direction, self.ADDR.nor)
                #return

            if (self.ADDR.alt < 0.0):
                SMOKES.gimme_a_fucking_smoke(self._pos, 4, 2.0, 'fire')
                self.expire = 1
        ...
        def draw(self):
            ...
            if self.type is 'laser':
                ...
                laserpos = self.target._pos if not self.interrupted else self.interrupted
                glColor4f(self.rgb[0], self.rgb[1], self.rgb[2], 1.0)
                w = 0.065

                self.density = self.rating

                glPushMatrix()
                glFrontFace(GL_CCW)
                s1 = USER._pos + (Vector3(USER.mat.right) * -w)
                s2 = USER._pos + (Vector3(USER.mat.right) * w)
                s3 = laserpos + (Vector3(USER.mat.right) * w)
                glBegin(GL_POLYGON)
                ...
                s1 = laserpos + (Vector3(USER.mat.right) * -w)
                s2 = USER._pos + (Vector3(USER.mat.right) * -w)
                s3 = laserpos + (Vector3(USER.mat.right) * w)
                glBegin(GL_POLYGON)
                glVertex3f(s1.x, s1.y, s1.z)
                ...
                #
                # if self.interruption_reflect is not 0:
                #     sref = laserpos+(self.interruption_reflect)
                #     glBegin(GL_LINES)
                #     glVertex3f(laserpos.x, laserpos.y, laserpos.z)
                #     glVertex3f(sref.x, sref.y, sref.z)
                #     glEnd()
                #
                #     #print(sref)
                #     # glPushMatrix()
                #     # glFrontFace(GL_CCW)
                #     s1 = sref + (Vector3(USER.mat.right) * w)
                #     s2 = sref + (Vector3(USER.mat.right) * -w)
                #     s3 = laserpos + (Vector3(USER.mat.right) * w)
                #     glBegin(GL_POLYGON)
                #     glVertex3f(s1.x, s1.y, s1.z)
                #     glVertex3f(s2.x, s2.y, s2.z)
                ...
                #
                #     s1 = laserpos + (Vector3(USER.mat.right) * -w)
                #     s2 = laserpos + (Vector3(USER.mat.right) * w)
                #     s3 = sref + (Vector3(USER.mat.right) * -w)
                #
                #     glBegin(GL_POLYGON)
                #     glVertex3f(s1.x, s1.y, s1.z)
                #     glVertex3f(s2.x, s2.y, s2.z)
                ...
                # # glVertex3f(laserpos.x, laserpos.y, laserpos.z)
                # # glEnd()
                # glTranslate(laserpos.x, laserpos.y, laserpos.z)
                # glColor4f(1.0, 1.0, 0.1, 1.0)
    ...
    def prep_laser(self, to_vector):
        """re-written to deal with non-ordered distance on poly group"""
        #grp = [g for g in WORLD.POLY_CENTERS_ARRAY[USER.ADDR.at_poly]['local_faces'] if BOX.testBounds(WORLD.POLY_CENTERS_ARRAY[g]['center'])]
        grp = WORLD.POLY_CENTERS_ARRAY[USER.ADDR.at_poly]['local_faces']
        comp = []
        for face in grp:
            poly_ver = WORLD.POLY_CENTERS_ARRAY[face]['hverts']
            poly_set = [Vector3(WORLD.obj.vertices[poly_ver[0]]),
                        Vector3(WORLD.obj.vertices[poly_ver[1]]),
                        Vector3(WORLD.obj.vertices[poly_ver[2]])]
            delt = (USER._pos - WORLD.POLY_CENTERS_ARRAY[face]['center'])
            case = intersect_test(USER._pos, to_vector, poly_set[0], poly_set[1], poly_set[2])
            if case: comp.append((delt,case,face))
        if len(comp) > 0:
            return min(comp)
        else:
            return 1
    ...
    def fire(self, shooter_obj, poly, flame_targeted, flame_speed, flame_type):
        ...
        flam.active = 1
        flam.speed = flame_speed #if flame_targeted is None else 1.0
        flam.rgb = shooter_obj.rgb
        flam.shooter = shooter_obj
        flam.frame = basic_flame_duration_frames if flame_targeted else basic_flame_duration_frames*2.0
        flam.frames = flam.frame



        if flame_targeted is None:
            flam.direction = (Vector3(shooter_obj.mat.forward)*-flame_speed)
            if shooter_obj.name != 'user': flam.direction = ((USER._pos-shooter_obj._pos).unit()*flame_speed)
            flam.rating = weapon_data['basic'] * shooter_obj.dmg
        else:
            flam.direction = Vector3(shooter_obj.mat.forward) * -0.1
            flam.rating = weapon_data['targeted'] * shooter_obj.dmg

        if flam.type == 'laser':
            flam.rating = weapon_data['laser'] * shooter_obj.dmg

            cq = USER._pos - flam.target._pos
            clear = self.prep_laser(cq)
            if clear != 1:
                if (clear[1][0] < 0) and (clear[1][0] > -1.0): #
                    dpos = (cq * clear[1][0])
                    flam.interrupted = USER._pos + dpos

                    #
                    # flam.interruption_reflect = flam.reflect(256.0*dpos, WORLD.POLY_CENTERS_ARRAY[clear[1]]['normal'])
                    # b_attitude_deg = 0
                    # try:
                    #     b_attitude_deg = (asin(flam.interruption_reflect.get_normalised().dot(Vector3(flam.target.mat.up))))
                    # except (ValueError, ZeroDivisionError) as t_error:
                    #     GEN_ERR(self.name, t_error)
                    #
                    # pht = tan(b_attitude_deg)*(flam.target._pos - flam.interrupted).length
                    # #stt = str(pht)[0:6]+ ' ' + str(flam.target.shield_radius)[0:6]
                    #
                    # if pht < 1.5*flam.target.shield_radius:
                    #     #stt = '%s in [%s]' % (str(pht*2.0)[0:5], str(flam.target.shield_radius*0.5)[0:5])
                    #     flam.interruption_reflect = flam.interruption_reflect.unit() * ((flam.target._pos-flam.interrupted).length)
                    #     flam.bounced_and_hit = 1

                    #TEXTB.draw(3.0, stt, USER._pos+flam.interruption_reflect, 0.5)
                    SMOKES.gimme_a_fucking_smoke(flam.interrupted, 2, 2.0, 'fire')

        # if flame_targeted is None:
        #     flam.direction = (Vector3(shooter_obj.mat.forward)*-flame_speed)
        #     if shooter_obj.name != 'user':
        #         flam.direction = ((USER._pos-shooter_obj._pos).unit()*flame_speed)
        # else:
        #     flam.direction = Vector3(shooter_obj.mat.forward)*-0.1
    ...
    def show(self):
        ...
        for f in self.flames:
            if f.target:
                ...
                if f.type == 'laser':
                    f.draw()
                    if not f.interrupted or f.bounced_and_hit:
                        f.target.flamed(f.index)
                        f.target.hitting = 1

                    # if f.bounced_and_hit:
                    #     f.target.flamed(f.index)
                    #     f.target.hitting = 1
...
class Special_Mine(object):
    ...
    def __init__(self):
        ...
        self.active = 0
        self.name = 'mine'
        Special_Mine.Mines.append(self)
        pass


    def place(self, placer_obj):
        self._pos = placer_obj.surface_position
        self.poly = placer_obj.ADDR.at_poly
        self.active = 1
        self.dropper = placer_obj
        pass

    @classmethod
    def idle(cls, time_delta):
        map(lambda u: u.show(time_delta), cls.Mines)


    def trigger(self):

        SMOKES.gimme_a_fucking_smoke(self._pos, 20, 3.0, 'fire')
        SMOKES.gimme_a_fucking_smoke(self._pos, 15, 1.0)
        self.active = 0
        f_end_mine = [Special_Mine.Mines.remove(mf) for mf in Special_Mine.Mines if not mf.active]

    def show(self, time_delta):
        ...
        if dt > self.timed:
            ...
            self.timed = dt
        if self.timer > 10.0:
            snd('explode-mini', self._pos)
            return self.trigger()

        try:
            hits = [ent.flamed(self) for ent in X29Element.PWEs
                    if not ent.killed and ent.name != self.dropper.name
                    and ent.ADDR.at_poly == self.poly
                    and ((ent._pos - self._pos).length < (ent.shield_radius))]
            if hits: return self.trigger()

        except ReferenceError as err:
            GEN_ERR(err, 'mine', self, 'orphaned')
            return self.trigger()
    ...
    def draw(self):
        TEXTB.draw(0.8, floor(self.timer), self._pos, 0.2)
        glPushMatrix()
        glFrontFace(GL_CCW)
        glTranslate(self._pos.x, self._pos.y, self._pos.z)
...
class CameraHandler(object):
    ...
    def update_pos(self, root_position):
        ...
        if self.mode == 'follow':
            ...
            #//STILL PROBLEMATIC
            USER.at_normal = Vector3(self.mat.up).normalize()
            USER.define_mat_from_normal()
            position = root_position.unit().normalize()
            #self.x_off = Vector3(self.mat.right)
...
class BlobEntityHandler(X29Element):
    ...
    def __init__(self, name):
        er = random.random() * 0.5 + 0.5
        super(BlobEntityHandler, self).__init__(name)
    ...
    def move_entity(self,delta_time):
...
class AggroEntityHandler(X29Element):
    #// WWORK HERE SECOND
    def __init__(self, name):
        #er = random.random() * 1.0 + 1.0
        super(AggroEntityHandler, self).__init__(name)
        # self.mass = er * 100.0
        # self.init_mass = er * 100.0
        self.type = 'aggro'
        # self.radius = 1.0
        # self.init_radius = 1.0
        self.graphic = aggro_d(self.radius)
        self.active = 1
        # self.rgb = (0.9, 0.9, 0.1)
        # self.ctk = 0.0
        # self.htk = 100.0
        # self.dmg = 10.0
        # self.shoots = 1
        # self.shield = 5.0
        # self.init_shield = 5.0
        # self.shield_radius = 5.0
        # self.targeting_enabled = 1
        self.delta_speak = 0
        self.timer = 0

    def speak_agknowledge(self, d_offset):
        threshold = 20.0
        if (d_offset < threshold) and (self.delta_speak > threshold):
            verb = random.choice(range(4))
            snd('aggro-speak-%i' % verb, self._pos)
            self.delta_speak = d_offset
        else:
            self.delta_speak = d_offset
    ...
    def move_entity(self,delta_time):
        ...
        if self.timer >= 2.0:
            self.timer = 0
            snd('aggro-drone',self._pos)

        self.define_mat_from_normal()
        try:
            pex = (USER._nextpos - self._pos)
            e_heading = Vector3(self.mat.forward) * -1.0
            e_right = Vector3(self.mat.right) * -1.0
            dir_rotate = pex.get_normalised().dot(e_right)
            rd = Vector3(0.0, dir_rotate, 0.0)
            self.apply_rotation_to_mat(rd)

            mov = e_heading * (2.0*movement_speed)
            #mov *= movement_speed_decay

            self.PHYS.set(mov)
            self.speak_agknowledge(pex.length)

            if self.shoots:
                try:
                    rf = int(random.random()*(20*pex.length))
                    if rf <= 2 and (pex.length < 30.0):
                        self.targeting = weakref.proxy(USER)
                        self.fire()
                except ValueError as err:
                    GEN_ERR(self.name,err,pex.length)
...
class ScoreMarker(object):
    ...
    def show(self):
        ...
        for s in self.scores:
            s.frames -= 1
            s.pos *= 1.001
            TEXTB.draw(s.size, str(s.points), s.pos, 0.0)
...
class Upgrade(object):
    ...
    def determine_type(self):
        ...
        if len(d) > 0: self.upgrade_type = random.choice(d)

        if self.upgrade_type == 'shield':
            s = (USER.shield / USER.init_shield)
            if s > 1.5:
    ...
    def intercept(self):
        if self.intercepted == 0:
            ...
            if self.upgrade_type != 'shield': upgrades[self.upgrade_type]['on'] = 1
            ...
            if self.upgrade_type == 'targeting_computer':
                USER.targeting_enabled = 1
                Score.msg("spacedar lock-on enabled")


            if self.upgrade_type == 'mines':
                Score.msg("use [%s] key to place a mine" % key_input['fire_special']['ks'])


            if self.upgrade_type == 'shield':
                USER.shield += USER.init_shield*0.5
                USER.shield_radius = USER.radius + (USER.shield/USER.init_shield)
                Score.msg("Shields increased to %d%%." % int((USER.shield/USER.init_shield)*100))
...
ent_lookup = {'blob':BlobEntityHandler,'aggro':AggroEntityHandler}


#//generator functions:
def generate_entities(criteria):
    for pc in criteria:
        for l in range(0, int(pc['_count'])):
            ENTT = ent_lookup[pc['_ent']](str(pc['_ent'])+'-entity-00' + str(l))
            for attr,v in pc.items():
                if attr[0] != '_':
                    setattr(ENTT, attr, int(v) if '.' not in v else float(v))
                    #ENTT.attr = int(v) if '.' not in v else float(v)

            ENTT.type = pc['_ent']
            ENTT.setup(float(pc['_variance']))

            #crit = ('shield','radius','shoots')
            #print [(e,getattr(ENTT,e)) for e in crit]


            ENTT.PHYS.set_position(ENTT.ADDR.init_random_poly() * 1.1)
            StackTrace("initialized %s" % ENTT.name)
            # StackTrace("initialized %s" % ENTT.name)
...
#
#
# def generate_blob_entities(howmany,predef):
#     """the class of red blobby npcs"""
#     for l in range(0, howmany):
#         ENTT = BlobEntityHandler('Blob-Entity-00' + str(l))
#         ENTT.index = l
#         for k,e in predef.items():
#             ENTT[k] = e
#         ENTT.PHYS.set_position(ENTT.ADDR.init_random_poly() * 1.5)
#     StackTrace("initialized %d NPCs" % howmany)
#
# def generate_aggro_entities(howmany,predef):
#     """the class of red blobby npcs"""
#     for l in range(0, howmany):
#         ENTT = AggroEntityHandler('Aggro-Entity-00' + str(l))
#         ENTT.index = l
#         for k,e in predef.items():
#             ENTT[k] = e
#         ENTT.PHYS.set_position(ENTT.ADDR.init_random_poly() * 1.5)
#     StackTrace("initialized %d Aggro NPCs" % howmany)
...
def draw_warp(index):
    ...
    a = (Vector3(WORLD.obj.vertices[ph[0]])*1.00001).as_tuple()
    b = (Vector3(WORLD.obj.vertices[ph[1]])*1.00001).as_tuple()
    c = (Vector3(WORLD.obj.vertices[ph[2]])*1.00001).as_tuple()
    glBegin(GL_POLYGON)
    glVertex3fv(a)
    glVertex3fv(b)
    glVertex3fv(c)
...
glLoadIdentity()
### M O D E L V I E W ###
syslog.syslog(syslog.LOG_ALERT,'BetterTypeHandler Culprit-check')

TEXTB = BetterTypeHandler(BASEDIR)

syslog.syslog(syslog.LOG_ALERT,'Loaded Classes')
...
def aggro_d(radius):
    aggro = glGenLists(1)
    glNewList(aggro, GL_COMPILE)
    glFrontFace(GL_CW)
    # bottom side
    glBegin(GL_TRIANGLES)
    a = Vector3(0.0, 0.0, (sqrt(3) / 3) * -2.0)
    b = Vector3(-1.0, 0.0, (sqrt(3) / 3))
    c = Vector3(1.0, 0.0, (sqrt(3) / 3))
    N = getNormal(a,b,c)*0.5
    glNormal3f(N.x, N.y, N.z)
    glVertex3f(a.x, a.y, a.z)
    glVertex3f(b.x, b.y, b.z)
    glVertex3f(c.x, c.y, c.z)
...
syslog.syslog(syslog.LOG_ALERT,'Scene ready(2)')
# ||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||

def fire_special():
    if upgrades['mines']['on']:
        n = Special_Mine()
        n.place(weakref.proxy(USER))
    else:
        snd('beep', 0.2)
...
def x29_world_init():
    ...
    X29Element.Waves_Killed = 0
    WORLD.load_world(init_world_size, current_level)
    StackTrace('WORLD.load_world(%s) COMPLETE' % init_world_size)
    USER.PHYS.set_position(USER.ADDR.init_random_poly() * 1.1)
    USER.targeting = None

    #for pwe in X29Element.PWEs: pwe.ADDR.update()
    #UPGRADES POLICE

    Upgrade.Upgrades_All = []
    for st in range(0, 15*model_world_density):
    ...
    rules = [r for r in runtime_table if (current_level >= int(r['_level'].split(',')[0])) and (current_level < int(r['_level'].split(',')[1]))]
    #print("rules", rules)


    generate_entities(rules)



    # generate_blob_entities(entct,rules)
    #
    #
    #
    #
    ...
    #
    # #if current_level > 0:
    # generate_aggro_entities(1,rules)
    # #AggroEntityHandler
    # StackTrace('finished spawning %d aggro' % 1)
...
tsec = 0.0
# ||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||
instruct = {1:instructions,2:instructions_2,3:instructions_3,4:['close']}

def x29_runtime(timedelta, has_input):
    state = 1
    global instruct, rotation_direction, movement_direction, ship_direction_vector, star_gl_list, GEN_ERR_MSG, tsec
    ...
    if run_x29.info:
        if run_x29.info < len(instruct):
            inst_position = (CAM._pos - Vector3(CAM.mat.forward)*4.0)+Vector3(CAM.mat.right)*2.0
            st = instruct[run_x29.info]

            since_dbg = 1
            if since_dbg:
                obs = {'GFPS': str(clock.get_fps())[0:2],
                       'GENE': GEN_ERR_MSG}
                lst = [kk+':  '+str(kv) for kk,kv in obs.items()]
                st += '\n'+('\n'.join(lst))
    ...
    for BLOB in X29Element.PWEs:
        if BLOB.name == 'user':
            ...
            BLOB.draw_entity()
            # glCallList(draw_utility_line(BLOB._pos, BLOB._pos + Vector3(BLOB.mat.up) * 2.0, (1.0, 0.0, 1.0)))
            # glCallList(draw_utility_line(BLOB._pos, BLOB._pos + Vector3(BLOB.mat.right) * 2.0, (1.0, 1.0, 0.0)))
    ...
    if USER.hitting:
        ...
        if USER.shield > 0.0:
            mms = (int((USER.shield / USER.init_shield) * 100))
            if USER.code == 0:
...
def x29_level_transition():
    ...
    for i in range(0, 100):
        ...
        sca = USER.prev_score + int(i*ceil(USER.score/100))
    ...
    bonuses = {'level':{'tot': 10000, 'mod':current_level, 'msg':'level multiplier'},
                ...
                'lethality': {'tot': 1000, 'mod':X29Element.Waves_Killed , 'msg': 'waves killed. bravo!'}}
    ...
    for bon,dat in bonuses.items():
        if dat['mod'] > 0:
            ...
            for i in range(0, 50):
                ...
                sca = int(i * (mod_score/float(50)))

                TEXTB.draw(0.5, message_str, run_x29.RENDER_POS, -0.25)
                TEXTB.draw(1.4, USER.score+sca, run_x29.RENDER_POS, 0.0)
                snd('ticker')
                pygame.display.flip()

            USER.score += mod_score
            snd('ticker', 'stop')
            snd('morphin', 0.45)
            pygame.time.wait(1500)

    glClear(GL_COLOR_BUFFER_BIT | GL_DEPTH_BUFFER_BIT)
    TEXTB.draw(1.3, random.choice(level_messages), run_x29.RENDER_POS, 0.0)
    pygame.display.flip()
...
if __name__ == "__main__":
    syslog.syslog(syslog.LOG_ALERT, 'Mains')
