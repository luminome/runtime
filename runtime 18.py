...
class WorldModelSet(object):
    ...
    def load_world(self, apex_nodes, level):
        ...
        for st in range(0,10):
            main_sector = int(random.randrange(0,len(WORLD.SUBSECTOR_ARRAY)))
            subs_sector = int(random.randrange(0, 3))
            vset = WORLD.SUBSECTOR_ARRAY[main_sector]['sectors'][subs_sector]['hverts']
            l_sector = int(random.randrange(len(vset)))
            d = vset[l_sector]

            f = random.choice([k for k in upgrades for dummy in range(upgrades[k]['rarity'])])
            f = f if not upgrades[f]['on'] else 'shield'
            upg = Upgrade(f, main_sector, subs_sector, d, WORLD.obj.vertices[d], WORLD.obj.normals[d])
...
class PhysicalWorldElement(object):
    ...
    def update(self, timepassed):
        ...
        try:
            ...
            # s_height = self.ADDR.alt
            # normal = self.ADDR.nor
            # poly = self.ADDR.thispoly
            ...
            #print(((current_position*s_height).length * current_position).length)
            #self.surface_position = s_height * current_position

            #ldl = (current_position.length-s_height) / current_position.length
            #self.surface_position = current_position*ldl# + s_height
            self.surface_position = s_height * current_position
            ...
            if (self.altitude < 0.2) and not self.PHYS.Jumpy:
                try:
                    ...
                    attitude_deg = degrees(asin(self.PHYS.Velo.copy().normalize().dot(nn)))
                except ValueError as degree_error:
                    self.visible_output_message((attitude_deg, degree_error),timepassed)

                self.PHYS.Pos = (self.surface_position.length + 0.2) * self.surface_position.copy().normalize()
...
class PolyAddressHandle(object):
    """referenced as .ADDR"""
    def __init__(self, parent_class_object):
        self._pos = Vector3()
        self.poly_a = Score_e()
        self.poly_b = Score_e()
        self.owner = parent_class_object
        self.thispoly = 0
        self.lastpoly = 0
        self.alt = 0
        self.nor = 0
    ...
    def init_random_poly(self):
        ...
        p = self.localize_poly(self.poly_a, r)
        print(self.owner.name, r,"verts:%s near(POLY_CENTERS):%s" % (str(p._ver), str(p._nea)))
        return p._pos

    def check(self, poly, pos):
        return point_in_poly(pos.normalize(), poly._set[0], poly._set[1], poly._set[2])
    ...
    def idle(self):
        self.message = 'None'
        p_current = self.check(self.poly_a, self.owner._pos)

        if not p_current:
            ct, pid, p_current, poly = self.get_new_poly(self.owner._nextpos)
            if pid:
        ...
        if p_current:
            self.thispoly = self.poly_a._id
            self.alt = (self.owner._pos - self.poly_a._nat).dot(self.poly_a._nor)
...
class ViewClipBox(object):
    ...
    def get_ground_interfere(self):
        ...
        for npoly in WORLD.POLY_CENTERS_ARRAY[poly]['nearest']:
...
class UserHandler(PhysicalWorldElement):
    ...
    def __init__(self, name):
        ...
        self.ADDR = PolyAddressHandle(self)
        self.mass = 150.0
...
class Upgrade(object):
    ...
    def __init__(self, upgradetype, sector, subsector, subpoly, poly_center, poly_normal):
        self.start_height = 1.01
        self.sector = sector
        self.poly = subpoly
        self.sub_sector = subsector
...
def draw_poly_hilight_group(poly):
    t = 1.001
...
def draw_sector_poly_hilight(index):
    ...
    vertices, normals, texture_coords, material = WORLD.obj.faces[index]
    glColor3f(0.2, 1.0, 0.2)
    a = WORLD.obj.vertices[vertices[0] - 1]
    b = WORLD.obj.vertices[vertices[1] - 1]
    c = WORLD.obj.vertices[vertices[2] - 1]
...
def draw_sub_sector_verts():
    ...
    vert_map = WORLD.POLY_RADII_ARRAY[USER.SCHD.sub_sector_poly_number]
...
def x29_world_init():
    ...
    # NPCs.generate_blob_entities(12)
...
def x29_runtime(timedelta, has_input):
    ...
    glCallList(draw_poly_hilight_group(USER.ADDR.poly_a._id))

    #user_poly = USER.SCHD.sub_sector_poly_number
    user_poly = USER.ADDR.thispoly
    #glCallList(draw_sector_poly_hilight(BOX.poly_confirm))
    glCallList(draw_sector_poly_hilight(USER.ADDR.poly_a._id))
    ...
    for fl in Flag.Flags:
        ...
        if fl.poly in WORLD.POLY_RADII_ARRAY[user_poly]:
    ...
    #//UPGRADES LIST
    ups = [u for u in Upgrade.Upgrades_All if u.poly in WORLD.POLY_RADII_ARRAY[user_poly]]
    for upg in ups:
        glCallList(upg.show())
